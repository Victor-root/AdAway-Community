...
- Render the GitHub release changelog as proper Markdown (headings, lists, bold, italic, inline code) instead of showing the raw markers
- Make the install flow robust to process death on memory-tight devices: the downloaded APK is preserved when the user is sent to the "install unknown apps" permission screen, and the install resumes from whichever Activity Android happens to restore (UpdateActivity or the launcher)
- Drive the install resume from both UpdateActivity and the launcher Activity so the install finishes on a single click even on devices that don't restore the original Activity stack
...
- Compact the Android TV home layout (status badge, hero text, button padding, row spacing) so the toggle, optional update banner and all four action rows fit on a single screen without scrolling on a standard TV viewport
